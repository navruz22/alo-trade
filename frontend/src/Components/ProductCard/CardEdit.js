  ...
  return (
    <div className="w-full border-t product-t flex m-0">
      <button
        ...
        className="w-full flex justify-center items-center border-r product-r py-1 "
      >
        <BsPencilSquare size={18} className="text-neutral-500" />
      </button>
      <button
        className=" w-full flex justify-center items-center py-1"
        onClick={() => deleteHandler(productId)}
      >
        <IoTrashOutline size={22} className="text-neutral-500" />
      </button>
    </div>
  );
};
